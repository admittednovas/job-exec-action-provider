...
// HandleGetSliTriggeredEvent handles get-sli.triggered events if SLIProvider == job-exec-action-provider
...
func HandleGetSliTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.GetSLITriggeredEventData) error {
	...
	if data.GetSLI.SLIProvider != "job-exec-action-provider" {
	...
	// Get SLI File from job-exec-action-provider subdirectory of the config repo - to add the file use:
	//   keptn add-resource --project=PROJECT --stage=STAGE --service=SERVICE --resource=my-sli-config.yaml  --resourceUri=job-exec-action-provider/sli.yaml
	sliFile := "job-exec-action-provider/sli.yaml"
...
func HandleActionTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.ActionTriggeredEventData) error {
	...
	if data.Action.Action == "recycleapppools-task" || data.Action.Action == "responsetimeclub-task" || data.Action.Action == "restartservice-task" {
		...
		startedEvent.SetExtension("shkeptncontext", myKeptn.KeptnContext)
